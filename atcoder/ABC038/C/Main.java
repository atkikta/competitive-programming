...
public class Main {
    ...
    void solve() throws IOException {
        ...
        Inchworm inchworm = new Inchworm(a); 
    ...
    class Inchworm {
        ...
        public long walk(){
            ...
            for (int left = 0; left < a.length; left++) {
                System.out.println(String.format("%s, %d %d","OUT", left, right));
                /* check if some condition is satisfied when right steps forward*/
                while(right < a.length && (right==left || a[right-1] < a[right])){
                    /* actually step forward */
                    right++;
                    System.out.println(String.format("%s, %d %d","OK", left, right)); 
                }

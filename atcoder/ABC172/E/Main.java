...
public class Main {
    ...
    void solve() throws IOException {
        int N = ni();
        int M = ni();
        ModComb mc = new ModComb(M+1, mod);
        int[] count = new int[N+1];
        for (int i = 1; i <=N ; i++) {
            int ret = 1;
            ret = mul(ret, mc.comb(N, i));
            ret = mul(ret, mc.perm(M, i));
            ret = mul(ret, mc.perm(M-i,N-i));
            ret = mul(ret, mc.perm(M-i,N-i)); 
            count[i] = ret;
        }
        int ans = mc.perm(M, N);
        ans = mul(ans, mc.perm(M,N));
        int compl = 0;
        for (int i = 1; i <= N; i++) {
            if(i%2==1){
                compl = add(compl, count[i]);
            }else{
                compl = sub(compl, count[i]);
            }
        }
        ans = sub(ans,compl);
        out.println(ans);

...
public class Main {
    ...
    void solve() throws IOException {
        ...
        int[][] csm = new int[N+1][N+1];
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < N; j++) {
                csm[i+1][j+1] = csm[i][j+1] + csm[i+1][j] - csm[i][j] + D[i][j];
            }
        }
        int[] maxv = new int[N*N+1];
        for (int sy = 0; sy <= N; sy++) {
            for (int ty = sy; ty <= N; ty++) {
                for (int sx = 0; sx <= N; sx++) {
                    for (int tx = sx; tx <= N; tx++) {
                        int res = csm[ty][tx] - csm[ty][sx] - csm[sy][tx] + csm[sy][sx];
                        maxv[(ty-sy)*(tx-sx)] = Math.max(res, maxv[(ty-sy)*(tx-sx)]);       
                    }
                }
            }
        }

...
public class Main {
    ...
    void solve() throws IOException {
        String xs = ns();
        long x = MathLib.carryDecimal(xs, 4);
        String ys = ns();
        long y = MathLib.carryDecimal(ys, 4);
        String zr = ns();
        long r = MathLib.carryDecimal(zr, 4);
        
        // System.out.println(String.format("%d %d %d", x, y, r));
        long maxY = floorToDecimal(y + r, dec);
        long minY = ceilToDecimal(y - r, dec);
        // System.out.println(String.format("%d %d", maxY, minY));
        long ans = 0;
        for (long yi = minY; yi <= maxY; yi+=dec) {
            long left = floorToDecimal(x, dec);
            long right = ceilToDecimal(x+r+1, dec);
            while(right-left>dec){
                long xi = (left/dec+right/dec)/2*dec;
                boolean isok = ((yi-y)*(yi-y)+(xi-x)*(xi-x) <= r*r);
                if(isok) left = xi;
                else right = xi;
            }
            long maxX = left;
            left = floorToDecimal(x-r-1, dec);
            right = ceilToDecimal(x, dec);
            while(right-left>dec){
                long xi = (left/dec+right/dec)/2*dec;
                boolean isok = ((yi-y)*(yi-y)+(xi-x)*(xi-x) <= r*r);
                if(isok) right = xi;
            ...
            if(maxX>=minX) ans += ((maxX - minX)/dec) + 1;
        }
        out.println(ans);
    }
    long ceilToDecimal(long x, long dec){
        if(x>0) return ((x+dec-1)/dec) * dec;
        else return (x/dec) * dec;
    }
    long floorToDecimal(long x, long dec){
        if(x>=0) return (x/dec) * dec;
        else return (x/dec-1) * dec;
    }
    final long dec = 10000;

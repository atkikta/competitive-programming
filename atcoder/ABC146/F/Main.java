...
public class Main {
    ...
    void solve() throws IOException {
        ...
        int M = ni();
        String s = ns();
        ArrayList<Integer> csumzero = new ArrayList<>();
        csumzero.add(0);
        for (int i = 0; i < N+1; i++) {
            csumzero.add( csumzero.get(i) + (s.charAt(i)=='0'?1:0) );
        }
        ArrayList<Integer> ans = new ArrayList<>();
        int at = N+1;
        BinarySearch<Integer> bs = new BinarySearch<>();
        System.err.println(csumzero);
        while(at>1){
            int left = Math.max(0,at - M - 1);
            int next = bs.upperBound(csumzero, csumzero.get(left));
            if(next == at) {
                out.println(-1);
                return;
            }

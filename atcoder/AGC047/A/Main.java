    ...
    void solve() throws IOException {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if(s.contains(".")){
                ...
                String[] spl = s.split(".");
                long intpart = Long.valueOf(spl[0]);
                int countzero = 0;
                for (int j = 0; j < 9; j++) {
                    if(spl[1].charAt(i) == '0') countzero++;
                    else break;
                }
                long frcpart = Long.valueOf(spl[1]);
                for (int j = 0; j < 9-countzero -1; j++) {
                    frcpart *= 10;
                }
                long val = intpart * 1L * 1000_000_000L + frcpart;
                ...
                int intpart = Integer.valueOf(s);
                HashMap<Integer,Integer> count = sieve.factorCount(intpart);
                twos[i] += count.getOrDefault(2, 0) + 9;
                fivs[i] += count.getOrDefault(5, 0) + 9;
            }
        }

...
public class Main {
    ...
    void solve() throws IOException {
        ...
        for (int i = 0; i <= N; i++) {
            for (int j = 0; j <= N; j++) {
                String conc = s.substring(0,i) + t.substring(j,N);
                if(conc.length()>=N){
                    if(conc.substring(0,N).equals(s) && 
                        conc.substring(conc.length()-N,conc.length()).equals(t)){
                            ans = Math.min(ans, conc.length());
                    }
                }
            }
        }

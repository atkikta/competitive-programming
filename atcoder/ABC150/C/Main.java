...
public class Main {
    ...
    void solve() throws IOException {
        ...
        for (int i = 0; i < list.size(); i++) {
            ArrayList<Integer> cand = list.get(i);
            boolean eqP = true;
            boolean eqQ = true;
            for (int j = 0; j < cand.size(); j++) {
                if(cand.get(j) != P[j]) eqP = false;
                if(cand.get(j) != Q[j]) eqQ = false;
            }
            if(eqP)idxP = i;
            if(eqQ)idxQ = i;
        }
        
        long ans = Math.abs(idxP-idxQ);
        out.println(ans);
    }
    int N;
    ...
    void dfs(ArrayList<Integer> state){
        if(state.size()==N) {
            list.add((ArrayList<Integer>)state.clone());
            return;
        }
        for (int i = 1; i <= N; i++) {
            if(!state.contains(i)){

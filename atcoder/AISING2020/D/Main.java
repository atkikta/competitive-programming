...
public class Main {
    ...
    void solve() throws IOException {
        ...
        int[] tensMpl = new int[N];
        tensMpl[0] = 1;
        for (int i = 1; i < N; i++) {
            tensMpl[i] = (tensMpl[i-1] * 2) % (pc+1);
        }
        int[] tensMmi = new int[N];
        tensMmi[0] = 1;
        for (int i = 1; i < N; i++) {
            if(pc>1)tensMmi[i] = (tensMmi[i-1] * 2) % (pc-1);
        }
        int mpl = 0;
        int mmi = 0;
        for (int i = 0; i < N; i++) {
        ...
        for (int i = 0; i < N; i++) {
            if(pc==1 && S.charAt(i)=='1'){
                out.println(0);
                continue;
            }
            int count = 1;
            if(S.charAt(i)=='0'){
                int m = (mpl + tensMpl[N-i-1]) % (pc+1);
                while(m>0){
                    m = m % Integer.bitCount(m);
                    count++;
                }
                out.println(count);
            }else{
                int m = (mmi + (pc-1)- tensMmi[N-i-1]) %  (pc-1);
                if(m < 0) m += (pc-1);
                if(m>=pc-1) m -= (pc-1);
                while(m>0){
                    m = m % Integer.bitCount(m);

...
public class Main {
    ...
    void solve() throws IOException {
        ...
        forward = new boolean[N-1];
        Arrays.fill(forward, false);
        edge_from = new int[N-1];
        edge_to = new int[N-1];
        for (int i = 0; i < N-1; i++) {
            int a = ni()-1;
            int b = ni()-1;
        ...
        dfs(0, -1);
        // System.err.println(Arrays.toString(forward));
        ...
        for (int i = 0; i < Q; i++) {
            ...
            if(forward[e] && t==2){
                c[edge_to[e]] += x;
            }else if(forward[e] && t==1){
                c[edge_to[e]] -= x;
                c[0] += x;
            }else if((!forward[e]) && t==2){
                c[edge_from[e]] -=x;
                c[0] += x;
            }else if((!forward[e]) && t==1){
                c[edge_from[e]] += x;
    ...
    void dfs(int node, int prev){
        for (Edge e : graph.get(node)) {
            if(e.to != prev){
                if(e.from == edge_from[e.id]) forward[e.id] = true; 
                dfs(e.to, node);
            }
        }
    }
